...
class ComidaDAO {
    ...
    protected static async grabeloYa(datos:Comida, res:Response): Promise<any>{
        ...
        .then(({queHacer, respuBase, creado})=>{
            switch(queHacer){
                case 1: 
                    res.status(200).json({respuesta: "Compita ya existe la Comida",
                        creado:creado,
                    });
                    break;
                default:
                    res.status(200).json({respuBase, creado:creado,
    ...
    protected static async borreloYa(datos: Comida, res: Response): Promise<any>{
        ...
        .task(async(consulta)=>{
            ...
            const existe = await consulta.one(SQL_COMIDAS.EXISTE_OTRA_TABLA, [datos.idComida]);
            if(existe.existe == 0){
                queHacer = 2;
                respuesta = consulta. result(SQL_COMIDAS.DELETE, [datos.idTipoComida]);
    ...
    protected static async actualiceloVarios(datos:Comida, res:Response): Promise<any>{
        await pool
        .task(async(consulta)=>{
            let queHacer = 1;
            let respuBase: any;
            ...
            if(cubi.existe != 0){
                ...
                respuBase = await consulta.none(SQL_COMIDAS.UPDATE_MASIVO, [datos.idTipoComida]);
                actualizado = true;
            }
    ...
    protected static async actualiceUno(datos:Comida, res:Response): Promise<any>{
        await pool
        .task(async(consulta)=>{
            let queHacer = 1;
            let respuBase: any;
